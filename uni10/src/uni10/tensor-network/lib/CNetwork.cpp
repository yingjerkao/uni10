...
namespace uni10{
CNode::CNode(): T(NULL), elemNum(0), parent(NULL), left(NULL), right(NULL), point(0){
}

CNode::CNode(CUniTensor* Tp): T(Tp), elemNum(Tp->m_elemNum), labels(Tp->labels), bonds(Tp->bonds), name(Tp->name), parent(NULL), left(NULL), right(NULL), point(0){
...
CNode::CNode(const CNode& nd): T(nd.T), elemNum(nd.elemNum), labels(nd.labels), bonds(nd.bonds), parent(nd.parent), left(nd.left), right(nd.right), point(nd.point){
}

CNode::CNode(std::vector<Bond>& _bonds, std::vector<int>& _labels): T(NULL), labels(_labels), bonds(_bonds), parent(NULL), left(NULL), right(NULL), point(0){
...
CNode CNode::contract(CNode* nd){
	...
	//CNode par(cBonds, newLabelC);
	return CNode(cBonds, newLabelC);
}

float CNode::metric(CNode* nd){	//Bigger is better
	int AbondNum = bonds.size();
	int BbondNum = nd->bonds.size();
	std::vector<Bond> cBonds;
	std::vector<int> markB(BbondNum, 0);
...
int64_t CNode::cal_elemNum(std::vector<Bond>& _bonds){
	int rBondNum = 0;
	int cBondNum = 0;
...
CNetwork::CNetwork(const std::string& fname): root(NULL), load(false), times(0), tot_elem(0), max_elem(0){
  try{
    fromfile(fname);
    ...
    leafs.assign(Tnum, (CNode*)NULL);
    tensors.assign(Tnum, (CUniTensor*)NULL);
  }
...
CNetwork::CNetwork(const std::string& fname, const std::vector<CUniTensor*>& tens): root(NULL), load(false), times(0), tot_elem(0), max_elem(0){
  try{
    fromfile(fname);
    ...
    for(int i = 0; i < Tnum; i++){
      ...
      CUniTensor* ten = new CUniTensor(*(tens[i]));
      ten->setName(names[i]);
      ten->setLabel(label_arr[i]);
      tensors[i] = ten;
      CNode* ndp = new CNode(ten);
      leafs[i] = ndp;
    }
    construct();
  }
...
void CNetwork::fromfile(const std::string& fname){//names, name2pos, label_arr, Rnums, order, brakets
	std::string str;
	std::ifstream infile;
...
void CNetwork::construct(){
	if(brakets.size()){
		std::vector<CNode*> stack(leafs.size(), NULL);
		int cursor = 0;
		int cnt = 0;
		for(int i = 0; i < brakets.size(); i++){
			if(brakets[i] < 0){
				for(int c = 0; c < -brakets[i]; c++){
					CNode* par = new CNode(stack[cursor - 2]->contract(stack[cursor - 1]));
...
void CNetwork::putTensor(size_t idx, const CUniTensor* UniT, bool force){
  try{
    ...
    if(leafs[idx] != NULL){
      *(tensors[idx]) = *UniT;
      ...
      CUniTensor* ten = new CUniTensor(*UniT);
...
void CNetwork::putTensor(const std::string& name, const CUniTensor* UniT, bool force){
  try{
...
void CNetwork::putTensorT(const std::string& nameT, const CUniTensor* UniT, bool force){
  try{
...
void CNetwork::branch(CNode* sbj, CNode* tar){
	CNode* par = new CNode(tar->contract(sbj));
...
void CNetwork::matching(CNode* sbj, CNode* tar){
...
CUniTensor CNetwork::launch(const std::string& _name){
  try{
    if(!load)
      construct();
    for(int t = 0; t < tensors.size(); t++)
...
CUniTensor CNetwork::merge(CNode* nd){
	if(nd->left->T == NULL){
		CUniTensor lftT = merge(nd->left);
		if(nd->right->T == NULL){
			CUniTensor rhtT = merge(nd->right);
...
size_t CNetwork::sum_of_memory_usage(){
  if(rollcall() >= 0)
    return 0;
  return _sum_of_tensor_elem(root) * sizeof(Complex);
}

size_t CNetwork::_sum_of_tensor_elem(CNode* nd) const{
  if(nd == NULL)
    return 0;
  return nd->elemNum + _sum_of_tensor_elem(nd->left) + _sum_of_tensor_elem(nd->right);
}

size_t CNetwork::memory_requirement(){
  if(rollcall() >= 0)
    return 0;
  size_t usage = 0;
  for(int i = 0; i < leafs.size(); i++)
  ...
  return max_usage * sizeof(Complex);
}

size_t CNetwork::_elem_usage(CNode* nd, size_t& usage, size_t& max_usage)const{
  if(nd == NULL)
    return 0;
...
size_t CNetwork::max_tensor_elemNum(){
  if(rollcall() >= 0)
    return 0;
  size_t max_num = 0;
  CNode max_nd;
...
void CNetwork::_max_tensor_elemNum(CNode* nd, size_t& max_num, CNode& max_nd) const{
  if(nd == NULL)
    return;
...
std::string CNetwork::profile(bool print){
  try{
...
void CNetwork::preprint(std::ostream& os, CNode* nd, int layer)const{
	if(nd == NULL)
		return;
...
std::ostream& operator<< (std::ostream& os, CNetwork& net){
  try{
    os<<std::endl;
...
std::ostream& operator<< (std::ostream& os, const CNode& nd){
...
void CNetwork::findConOrd(CNode* nd){
