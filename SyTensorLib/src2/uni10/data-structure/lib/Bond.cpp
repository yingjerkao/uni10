...
//namespace uni10{
//namespace datatype{
Bond_t::Bond_t(bondType _type, std::vector<Qnum>& qnums) : type(_type){
	setting(qnums);
}

void Bond_t::assign(bondType _type, std::vector<Qnum>& qnums){
	type = _type;
	Qnums.clear();
	Qdegs.clear();
	offsets.clear();
...
void Bond_t::setting(std::vector<Qnum>& qnums){
	assert(qnums.size() > 0);
